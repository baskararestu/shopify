  ...
  sendVerificationEmail: async (nodemailer, email, fullName, otp, token) => {
    const mailOptions = {
      ...
      <div>
        ...
        <p>Verification Link: <a href="https://jcwdol0910.purwadhikabootcamp.com/verification/?email=${email}&token=${token}">Click here to verify</a></p>
  ...
  sendResetPasswordEmail: async (nodemailer, email, token) => {
    const mailOptions = {
      ...
      <div>
        ...
        <h2><a href="https://jcwdol0910.purwadhikabootcamp.com/reset-password/?email=${email}&token=${token}">Click Here</a></h2>
      </div>
    `,
    };
    const response = await nodemailer.sendMail(mailOptions);
